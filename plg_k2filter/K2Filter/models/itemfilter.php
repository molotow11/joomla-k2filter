...
class K2ModelItemFilter extends K2Model
{
	...
	function prepareItem($item, $view, $task) {
		...
		$category_data->params = $category->params;
		...
		else if(JFile::exists(JPATH_SITE.'/media/k2/galleries/'.$item->id.'/'.$item->id.'.jpg')) {
			$item->imageXSmall = JURI::root(true).'/media/k2/galleries/'.$item->id.'/'.$item->id.'.jpg';
        }
		...
		|| ($view=='itemlist' && $task=='filter' && ($item->params->get('genericItemExtraFields') || $item->params->get('catItemExtraFields')))
		...
		)
		{
			...
			if($template != "ja_wall") {
				$item->extra_fields = K2ModelItemFilter::getItemExtraFields($item->extra_fields, $item);
			}
			//added
		}
	...
	function execPlugins($item, $view, $task) {
		...
		if(
			($view=='item' && $item->params->get('itemExtraFields')) ||
			($view=='itemlist' && ($task == '' || $task == 'category' || $task == 'filter') && $item->params->get('catItemExtraFields'))
		) {
	...
	function getItemExtraFields($itemExtraFields, &$item = null)
	{
		...
		{
			K2ModelItemFilter::buildAliasBasedExtraFields($K2ItemExtraFieldsInstances[$itemExtraFields], $item);
