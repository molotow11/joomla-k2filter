	...
	jQuery(document).ready(function() {
		...
		jQuery(document).ready(function() {
			jQuery.fn.highlight = function (str, className) {
				...
				return "<span style='background-color: #FFFFCC; font-weight: bold; padding: 1px 4px;' class=\"" + className + "\">" + match + "</span>";
						});
					});
				});
			};
			<?php if(JRequest::getVar("fitem_all", "") != "") : ?>
			jQuery(".genericItemView *, .catItemView *").highlight("<?php echo JRequest::getVar("fitem_all", ""); ?>", "highlight");
			<?php endif; ?>
		});
			  ...
			  <div class="genericItemIntroText">
				<?php
					if(JRequest::getVar("fitem_all") != "") {
						$searchword = JRequest::getVar("fitem_all");
						
						// Strips tags won't remove the actual jscript
						$text = preg_replace("'<script[^>]*>.*?</script>'si", "", $item->introtext);
