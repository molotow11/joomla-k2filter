...
class K2ViewItem extends K2View
{
	...
	function display($tpl = null)
	{
		...
		if ($item->params->get('itemNavigation'))
		{
			...
			$get_vars = explode("&", $lastsearch);
			$get_new = Array();
			foreach($get_vars as $var) {
				$var = explode("=", $var);
				$var_key = explode("[", $var[0]);
				$var_key_type = preg_replace('/[0-9]+/', '', $var_key[0]);

				switch($var_key_type) {
					case "array":
					case "category":
					case "taga":
						$get_new[$var_key[0]][] = $var[1];
					break;
			...
			JRequest::setVar("getSiblingsForNavigation", 1);
			JRequest::setVar("articleId", $item->id);
			$siblings = $model->getData();

			$nextItem = $siblings->nextItem;
			if (!is_null($nextItem))
			{
				$item->nextLink = urldecode(JRoute::_(K2HelperRoute::getItemRoute($nextItem->id.':'.urlencode($nextItem->alias), $nextItem->catid.':'.urlencode($item->category->alias)))) . "?fromsearch=1";;
			...
			$previousItem = $siblings->prevItem;
			if (!is_null($previousItem))
			{
				$item->previousLink = urldecode(JRoute::_(K2HelperRoute::getItemRoute($previousItem->id.':'.urlencode($previousItem->alias), $previousItem->catid.':'.urlencode($item->category->alias)))) . "?fromsearch=1";;
