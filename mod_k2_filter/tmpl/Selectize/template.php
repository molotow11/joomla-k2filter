...
<script>
	jQuery(document).ready(function() {
		...
		jQuery("#K2FilterBox<?php echo $module->id; ?>").find("select").each(function() {
...
<div id="K2FilterBox<?php echo $module->id; ?>" class="K2FilterBlock selectize <?php echo $params->get('moduleclass_sfx'); ?><?php if($cols == "0") echo ' k2filter-responsive'; ?>">
...
?>		<div class="clear" style="clear: both;"></div>
	...
	<?php if($restrict == 1) : ?>
		<?php if($restmode == 1) : ?>			
			<?php 
				...
				if($view == "itemlist" && JRequest::getInt("id") != "") { 
					$restcata[] = JRequest::getInt("id");
				}
				else if($view == "item") {
					$id = JRequest::getInt("id");
				...
				if(!count($restcata)) {
					...
					if($menu) {
						if(count($menu->params->get('categories'))) {
							$restcata = array_merge($restcata, $menu->params->get('categories'));
						}
					}
				}
			?>
			...
			<?php if($restcata != "") : ?>
				<input type="hidden" name="restcata" value="<?php echo implode(",", $restcata); ?>" />
			<?php endif; ?>
	...
	<?php if(JRequest::getInt('featured') == 2) { ?>
	...
	<div class="buttons">
		...
		<?php endif; ?>
		<?php if ($clear_btn) require(JModuleHelper::getLayoutPath('mod_k2_filter', $getTemplate.DS.'elements/clear_btn')); ?>
	</div>
  </form>
