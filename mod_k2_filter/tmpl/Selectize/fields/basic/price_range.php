...
<script type="text/javascript">
	...
	<?php 
		$from = JRequest::getVar("price-from", $field->content[0]);
		$to = JRequest::getVar("price-to", $field->content[$values_count]);
		$value = $from. " - " .$to;
	?>
	...
	<div class="k2filter-field-price-range">
		...
		<div class="slider<?php echo $module->id.$k;?>_wrapper">
			...
			<input id="price<?php echo $k; ?>-from" class="slider_val" type="hidden" name="price-from" value="<?php echo JRequest::getVar("price-from") != 0 ? $from : ''; ?>">
			<input id="price<?php echo $k; ?>-to" class="slider_val" type="hidden" name="price-to" value="<?php echo JRequest::getVar("price-to") != 0 ? $to : ''; ?>">
